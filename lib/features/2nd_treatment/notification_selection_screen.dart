...
class _NotificationSelectionScreenState
  ...
  void initState() {
    ...
    _abcId = widget.abcId; 
    _loadExisting();
    debugPrint('[NOTI] _origni=$_origin');
  }
  ...
  Future<void> _onSavePressed() async {
    ...
    if (_noNotification) {
      ...
      if (_origin == 'training') {
        navigator.pushNamed('/breath_muscle_relaxation');
      } else if (_origin == 'apply') {
          final uid = FirebaseAuth.instance.currentUser?.uid;
            int completed = 0;
            if (uid != null) {
    ...
    if (_origin == 'training') {
      ...
      final counter = context.read<UserDayCounter>();
      final week = counter.getWeekNumberFromJoin(DateTime.now());
