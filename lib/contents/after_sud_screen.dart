...
class _AfterSudRatingScreenState extends State<AfterSudRatingScreen> {
  ...
  Future<void> _compareAndNavigate() async {
    ...
    final List<String> allBList =
        (args['allBList'] as List?)?.cast<String>() ?? const [];
    final List<String> remainingBList =
        (args['remainingBList'] as List?)?.cast<String>() ?? const [];
    final List<String> allAlternativeThoughts =
        (args['allAlternativeThoughts'] as List?)?.cast<String>() ?? const [];
        ...
        children: [
          Padding(
            ...
            child: Column(
              ...
              children: [
                Text(
                  '대체 생각 작성 이후, 지금 느끼는 불안 정도를 슬라이드로 선택해 주세요.',
